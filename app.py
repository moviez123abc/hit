...
def delete_sheet(id):
    try:
        # ১. আইডি চেক এবং অবজেক্ট আইডিতে রূপান্তর
        if not ObjectId.is_valid(id):
            flash('ইনভ্যালিড আইডি ফরম্যাট', 'danger')
            return redirect(url_for('index'))
            
        query_id = ObjectId(id)

        # ২. প্রথমে চেক করুন শীটটি আসলে আছে কি না
        target_sheet = db.sheets.find_one({"_id": query_id})
        
        if target_sheet:
            # ৩. এই শীটের আন্ডারে থাকা সকল কাস্টমার ডিলিট করুন
            # (ধরে নিচ্ছি কাস্টমার কালেকশনে sheet_id নামে রেফারেন্স আছে)
            db.customers.delete_many({"sheet_id": query_id})
            
            # ৪. মূল শীটটি ডিলিট করুন
            db.sheets.delete_one({"_id": query_id})
            
            flash(f"'{target_sheet.get('group_name')}' শীট এবং এর আওতাধীন সকল তথ্য ডিলিট হয়েছে।", 'success')
        else:
            flash('শীটটি খুঁজে পাওয়া যায়নি!', 'danger')
...
def edit_customer(cust_id):
    try:
        # ১. ObjectId কনভার্ট করে ডাটা খোঁজা
        customer = customers_col.find_one({"_id": ObjectId(cust_id)})
        
        # শিট লিস্ট নিয়ে আসা (যদি ড্রপডাউন থাকে)
        all_sheets = list(sheets_col.find({}, {"group_name": 1}))
        
        if customer:
            # ২. ডিফল্ট ভ্যালু সেট করা (TypeError এড়াতে)
            ...
            # ৩. সঠিক টেমপ্লেট ফাইলটি কল করুন (অবশ্যই edit_customer.html হতে হবে)
...
def update_customer(cust_id):
    try:
        # ১. বেসিক তথ্য (নাম, এসি নং এগুলো যা আছে তাই থাকবে বা আপডেট হবে)
        sheet_id = request.form.get('sheet_id')
        name = request.form.get('name')
        acc_no = request.form.get('acc_no')
        
        # ২. শুধুমাত্র বেসিক ফিল্ডগুলো $set এ রাখুন
        update_data = {
            "$set": {
                "customer_name": name,
                "acc_no": acc_no,
        ...
        # ৩. নতুন এক্সট্রা পণ্যের তথ্য সংগ্রহ
        ...
        if new_p_name:
            ...
            # এখানে $push ব্যবহার করায় এটি আগের ডাটা না মুছে শুধু লিস্টে যোগ হবে
            update_data["$push"] = {"extra_products": new_entry}

        # ৪. ডাটাবেস আপডেট (মেইন product_name এখন আর চেঞ্জ হবে না)
