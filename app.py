...
def save_kisti(sheet_id):
    try:
        # ১. শীটের অধীনে থাকা সকল কাস্টমার ডাটাবেস থেকে নিয়ে আসা
        customers = list(customers_col.find({"sheet_id": sheet_id}))
        
        for customer in customers:
            cid = str(customer['_id'])
            
            # ২. ফর্ম থেকে ডাটা সংগ্রহ করার সময় 'safe conversion' নিশ্চিত করা
            # request.form.get(..., 0) এর সাথে float() ব্যবহার করা হয়েছে যাতে স্ট্রিং এরর না আসে
            def get_val(field_name):
                val = request.form.get(f'{field_name}_{cid}', '0').strip()
                return float(val) if val else 0.0

            # কিস্তি এবং মূল্য আদায়ের লিস্ট তৈরি (১ থেকে ৫ পর্যন্ত)
            kisti_data = [get_val(f'n_{i}') for i in range(1, 6)]
            price_data = [get_val(f'm_{i}') for i in range(1, 6)]
            
            # ৩. ডাটাবেসে আপডেট করার জন্য অবজেক্ট তৈরি
            update_data = {
                "duration_months": request.form.get(f'duration_{cid}'),
                "product_name": request.form.get(f'item_name_{cid}'),
                "delivery_date": request.form.get(f'dist_date_{cid}'),
            ...
            # ৪. কাস্টমার ভিত্তিক আপডেট কুয়েরি চালানো
            customers_col.update_one({"_id": ObjectId(cid)}, {"$set": update_data})
        
        flash("কিস্তি শিট সফলভাবে আপডেট করা হয়েছে!", "success")
        
    except Exception as e:
        # এরর ডিবাগিং এর জন্য কনসোলে প্রিন্ট করা
