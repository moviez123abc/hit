...
def update_customer(cust_id):
    try:
        ...
        # ২. নতুন পণ্যের তথ্য (যদি ইউজার ইনপুট দেয়)
        new_p_name = request.form.get('new_product')
        new_p_price = float(request.form.get('new_price') or 0)
        new_p_profit = float(request.form.get('new_profit') or 0)
        new_p_kisti = float(request.form.get('new_per_kisti') or 0)
        new_p_date = request.form.get('new_date') or datetime.now().strftime("%Y-%m-%d")
        ...
        # যদি নতুন পণ্যের নাম থাকে, তবেই সেটি extra_products লিস্টে যাবে
        if new_p_name:
            new_entry = {
                "p_name": new_p_name,
                "p_price": new_p_price,
                "p_profit": new_p_profit,
                "p_kisti": new_p_kisti,
                "added_date": new_p_date
            }
            update_data["$push"] = {"extra_products": new_entry}
            
            # মেইন ডিসপ্লে ফিল্ডগুলোও লেটেস্ট পণ্য দিয়ে আপডেট করে দিচ্ছি
            update_data["$set"].update({
                "product_name": new_p_name,
                "cost_price": new_p_price,
                "profit": new_p_profit,
                "per_kisti": new_p_kisti,
                "delivery_date": new_p_date
            })

        customers_col.update_one({"_id": ObjectId(cust_id)}, update_data)
        flash("তথ্য সফলভাবে আপডেট করা হয়েছে!", "success")

        # ৪. রিডাইরেক্ট (Error Handle সহ)
        if sheet_id:
            return redirect(url_for('view_kisti_sheet', sheet_id=sheet_id))
        return redirect(url_for('manage_customers_page'))
