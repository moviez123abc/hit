...
users_col = db['Users']
kisti_updates_col = db['kisti_updates']
...
def login():
    if request.method == 'POST':
        ...
        if user:
            try:
                # ডিভাইস এবং নেটওয়ার্ক ডাটা সংগ্রহ
                ua_string = request.headers.get('User-Agent')
                user_agent = parse(ua_string)
                current_ip = request.remote_addr
                
                # বর্তমানে যে ডাটাগুলো ওভাররাইট হবে
                tracking_data = {
                    "last_login": datetime.now().strftime("%Y-%m-%d %I:%M %p"),
                    "device_model": user_agent.device.family or "Unknown",
                ...
                # ডাটাবেস আপডেট: 
                # $set -> বর্তমান তথ্য ওভাররাইট করবে
                # $addToSet -> নতুন আইপি হলে security_ips লিস্টে যোগ করবে (ডুপ্লিকেট হবে না)
                db.Users.update_one(
                    {"_id": user['_id']},
                    {
                        "$set": tracking_data,
                        "$addToSet": { "security_ips": current_ip }
                    }
                )
...
def device_logs():
    try:
        # যাদের current_ip ফিল্ড আছে তাদের লিস্ট আনা
        users = list(db.Users.find({"current_ip": {"$exists": True}}))
        
        return render_template('admin_device_logs.html', users=users)
    except Exception as e:
        print(f"Error loading logs: {e}")
        flash("লগ লোড করতে সমস্যা হয়েছে।", "danger")
        return redirect(url_for('index'))

@app.route('/admin/reset_tracking/<user_id>', methods=['POST'])
def reset_tracking(user_id):
    try:
        # ইউজারের ট্র্যাকিং ডাটা এবং আইপি লিস্ট মুছে ফেলা
        db.Users.update_one(
        ...
        return {"status": "success", "message": "ট্র্যাকিং ডাটা রিসেট করা হয়েছে।"}, 200
    except Exception as e:
...
def save_customer(): 
    try:
        ...
        if not sheet_id:
            flash("দয়া করে একটি দল নির্বাচন করুন!", "danger")
            return redirect(url_for('add_customer_page'))

        # ডাটা স্ট্রাকচার তৈরি (আপনার নতুন ফর্মের সকল ফিল্ড অনুযায়ী)
        customer_data = {
            "sheet_id": sheet_id,
            ...
            "total_price": float(request.form.get("total_price") or 0), # সার্ভিস চার্জসহ মূল্য
            "per_kisti": float(request.form.get("per_kisti") or 0),    # সাপ্তাহিক মূল্য
            "total_kisti": int(request.form.get("total_kisti") or 0), # মোট সপ্তাহ
            "comment": request.form.get("comment", ""),   # ভর্তি/ফরম/বিবিধ
            
            # ট্র্যাকিং ও স্ট্যাটাস
            "added_by": session.get('user'),
            "entry_date": datetime.now(),
            "status": "approved" if role == 'admin' else "pending",
            
            # কালেকশন এবং বকেয়া জের
            "extra_products": [],
            "collections": {
                "pre_due_n": float(request.form.get("pre_due_n") or 0), # নগদ বকেয়া জের
                "pre_due_m": float(request.form.get("pre_due_m") or 0), # মূল্য বকেয়া জের
                "kisti_data": [], 
                "price_data": [], 
                "return_cash": 0,
                "discount": 0,
        ...
        if role == 'admin':
            flash("নতুন সদস্য এবং আর্থিক বিবরণ সফলভাবে যুক্ত হয়েছে!", "success")
        else:
            flash("সদস্য যুক্ত হয়েছে এবং অ্যাডমিনের অনুমোদনের জন্য অপেক্ষমান।", "info")
...
def print_kisti_sheet(sheet_id):
    try:
        ...
        col_dates = sheet.get('manual_dates', ['১ম সপ্তাহের তারিখ ', '২য় সপ্তাহের তারিখ ', '৩য় সপ্তাহের তারিখ ', '৪র্থ সপ্তাহের তারিখ ', '৫ম সপ্তাহের তারিখ '])
...
def edit_customer(cust_id):
    try:
        ...
        if customer:
            ...
            customer['acc_no'] = customer.get('acc_no', '')
            customer['join_date'] = customer.get('join_date', '')
            
            customer['product_name'] = customer.get('product_name', '')
            customer['delivery_date'] = customer.get('delivery_date', '')
            ...
            customer['total_kisti'] = customer.get('total_kisti', '')
